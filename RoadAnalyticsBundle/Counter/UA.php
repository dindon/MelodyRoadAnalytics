...
use Melody\RoadAnalyticsBundle\Counter\Spyc;
/*!
	...
	*/
	public static function get() {
		if ($data = @file_get_contents("https://raw.github.com/tobie/ua-parser/master/regexes.yaml")) {
			if (file_exists(__DIR__."/resources/config/regexes.yaml")) {
				if (!self::$nobackup) { 
					if (!self::$silent) { print("backing up old YAML file...\n"); }
					if (!copy(__DIR__."/resources/config/regexes.yaml", __DIR__."/resources/regexes.".date("Ymdhis").".yaml")) {
						if (!self::$silent) { print("back-up failed...\n"); }
						exit;
					}
				}
			}
			$fp = fopen(__DIR__."/resources/config/regexes.yaml", "w");
			fwrite($fp, $data);
			fclose($fp);
